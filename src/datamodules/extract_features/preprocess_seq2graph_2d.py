...
class TestDataset(Dataset):
    ...
    def padding(self, img):
        ...
        assert desired_size_row > img.shape[0] or desired_size_col > img.shape[
            1], "the patch dimension is bigger than image shape"
    ...
    def preprocess_features_metric_learning(self, path_to_write, dict_path):
        # dict_path = r"D:\Python\cell-tracker-gnn-main\data\software\parameters\Features_Models\Fluo-N2DH-SIM+\all_params.pth"
        # print(os.path.abspath(dict_path))  # 打印绝对路径，确认路径正确
        ...
        for ind_data in range(self.__len__()):
            ...
            for ind, id_res in enumerate(np.unique(result)):
                ...
                embedded_feat = self.extract_freature_metric_learning(properties.bbox, img.copy(), result.copy(),
                                                                      id_res)
...
def create_csv(input_images, input_masks, input_seg,
    ...
    # print("input_model: ", os.path.abspath(input_model))
...
if __name__ == "__main__":
    ...
    parser.add_argument('-im', type=str, help='metric learning model params directory')
