...
class Postprocess(object):
    ...
    def find_connected_edges(self):
        ...
        if not self.directed:
            final_outputs_hard, edge_index = self.megre_match_edges(edge_index.detach().clone(),
            ...
            for i in nodes_indices:
                if i in connected_indices[0, :]:
                    ...
                    if ind_place.shape[-1] > 1:
                        ...
                        if self.is_3d:
                            next_frame = df.loc[
                                next_frame_ind, ["centroid_depth", "centroid_row", "centroid_col"]].values
        ...
        for idx in range(n_rows):
            ...
            for id, true_id in zip(graph_ids, graph_true_ids):
                ...
                if self.is_3d:
                    ...
                    if val == 0:
                        if np.any(pred[depth_center - 3:depth_center + 3, row_center - 3:row_center + 3,
                                  col_center - 3:col_center + 3] != 0):
                            area = pred[depth_center - 3:depth_center + 3, row_center - 3:row_center + 3,
                            ...
                            print(df.loc[
                                      id, ["seg_label", "frame_num", "centroid_depth", "centroid_row", "centroid_col",
                                           "min_depth_bb",
                                           "min_row_bb", "min_col_bb", "max_depth_bb", "max_row_bb",
                                           "max_col_bb"]].astype(int))
                            print()
                            continue
                else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('-modality', type=str, default="2D", help='2D/3D modality')
    parser.add_argument('-iseg', type=str, default="../datasets/Fluo-N2DH-SIM+/Test/01_SEG",  # 原始分割图片目录
                        help='segmentation output directory')
    parser.add_argument('-oi', type=str, default="../datasets/Fluo-N2DH-SIM+/Test/01_RES_inference",  # 生成的推理文件目录
                        help='inference output directory')  # 最终生成的追踪分割文件和res_track.txt位于 iseg同级目录下
